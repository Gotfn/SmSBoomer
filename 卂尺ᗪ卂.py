...
def watermark():
    print("SMS Boomer v1.0 / 卂尺ᗪ卂 Tarafından Kodlanmıştır")

def get_number():
    global clear
    while True:
        try:
            number = int(input(f"""Telefon numarasını yazın. Şunun gibi: "54xxxxxxxx"\n[?] : """))
...
def get_worker_amount():
    ...
    while True:
        try:
            worker_amount = int(input(f"Kaç saniyede bir gönderilsin. Tavsiye edilen 5-100 arasıdır.\n[?] : "))
            if worker_amount >= 1:
